...
class SelectCodes extends Component {
    constructor(props){
        ...
        this.searchTermDescription = this.searchTermDescription.bind(this)
    }
    componentDidMount(){
    ...
    filterDescription(targetValue){
        let searchTerm = targetValue.toLowerCase()
        let filteredCodes = this.props.codes.filter(obj => obj.description.toLowerCase().startsWith(searchTerm, 0))
        this.setState({filteredCodes:filteredCodes})
    }
    searchTerm(e){
        e.preventDefault()
    ...
    searchTermDescription(e){
      e.preventDefault()
      console.log(e.target.value)
      this.setState({
        searchTerm:e.target.value
      }, this.filterDescription(e.target.value))
    }
    selectMaterial(e){
        e.preventDefault()
        const materialGroups = () => {
            if(this.props.codes){
                return ({
                    ...
                    Cloth:this.props.codes.filter(obj => obj.material === "Cloth"),
                    Chemicals:this.props.codes.filter(obj => obj.material === "Chemicals")
                })
            }
        }
        this.setState({
    ...
    addToInventory(e){
        e.preventDefault()
        let codeAndQuantity = {name:"codeAndQuantity", code:this.state.selectedCode, description:this.state.selectedDescription, quantity:this.state.codeQuantity}
            ...
            <div className="inner-column-div">
                ...
                <div className="row-wrap">
                    <label className="display-inline-b label-position label-format">
                        <h6 className="">Search by code:</h6>
                        <input
                            className="inputTall"
                            type="number"
                            id="codeSearch"
                    ...
                    <label className="display-inline-b label-position label-format">
                        <h6 className="">Search by description:</h6>
                        <input
                            className="inputTall"
                            type="text"
                            ...
                            value={this.state.searchTerm || ""}
                            onChange={this.searchTermDescription}
                        />
                    </label>
                </div>
                <motion.div
                ...
                <div className="row-wrap" >
                    {
                        this.state.selectedCode ?
                          <>
                            <h6 className="">Selected: {this.state.selectedCode}</h6>
                            <label className="display-inline-b label-position label-format">
